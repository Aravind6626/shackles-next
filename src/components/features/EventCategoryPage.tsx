  ...
  async function handleJoinTeam() {
    ...
    try {
      const res = await fetch('/api/events/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ eventName: selectedEvent.name, teamCode: code, action: 'jointeam' }),
      });
      const data = await res.json();
      if (!res.ok) {
        setFeedbackType('error');
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="shrink-0 rounded-lg bg-gray-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              Create
                            </button>
                          </div>
